...
const outerContainerBackgroundColor = $(outerContainerBackgroundColor:Color="#FFFFFF");
...
const innerContainerBackgroundColor = $(innerContainerBackgroundColor:Color="#FFFFFF");
...
const innerContainerRoundedCorners = $(innerContainerRoundedCorners:Dropdown[None|rounded-none,Small|rounded-sm,Medium|rounded-md,Large|rounded-lg,Extra Large|rounded-xl]="rounded-md");
const innerContainerSecondaryBackgroundColor = $(innerContainerSecondaryBackgroundColor:Color="#C5AD96");
// endgroup

// group: { 3.1 Inner Section Content }
const eyebrowContent = $(eyebrowContent:RichText="";color="#000000");
const headlineContent = $(headlineContent:RichText="";color="#000000");
const subheadContent = $(subheadContent:RichText="";color="#000000");
const ctaContent = $(ctaContent:RichText="";color="#000000");
const ctaUrl = $(ctaUrl:String="/";required=true);
const ctaAdobeTag = $(ctaAdobeTag:String="";required=true);
// endgroup

// group: { 3.2 Secondary CTA }
const secondaryCtaContent = $(secondaryCtaContent:RichText="";color="#000000");
const secondaryCtaUrl = $(secondaryCtaUrl:String="/";required=true);
const secondaryCtaAdobeTag = $(secondaryCtaAdobeTag:String="";required=true);
// endgroup

// group: { 4. Card Layout }
const desktopCardAspectRatioWidth = $(desktopCardAspectRatioWidth:String="580");
const desktopCardAspectRatioHeight = $(desktopCardAspectRatioHeight:String="656");
const tabletCardAspectRatioWidth = $(tabletCardAspectRatioWidth:String="580");
const tabletCardAspectRatioHeight = $(tabletCardAspectRatioHeight:String="656");
const mobileCardAspectRatioWidth = $(mobileCardAspectRatioWidth:String="180");
const mobileCardAspectRatioHeight = $(mobileCardAspectRatioHeight:String="150");
const cardRoundedCorners = $(cardRoundedCorners:Dropdown[None|rounded-none,Small|rounded-sm,Medium|rounded-md,Large|rounded-lg,Extra Large|rounded-xl]="rounded-md");
// endgroup

// Derived aspect-ratio fallbacks to ensure images render even when CMS inputs are empty
const desktopContainerAspectRatioWidthValue = withFallback(desktopContainerAspectRatioWidth, "1440");
const desktopContainerAspectRatioHeightValue = withFallback(desktopContainerAspectRatioHeight, "540");
const tabletContainerAspectRatioWidthValue = withFallback(tabletContainerAspectRatioWidth, "960");
const tabletContainerAspectRatioHeightValue = withFallback(tabletContainerAspectRatioHeight, "600");
const mobileContainerAspectRatioWidthValue = withFallback(mobileContainerAspectRatioWidth, "375");
const mobileContainerAspectRatioHeightValue = withFallback(mobileContainerAspectRatioHeight, "640");
const desktopCardAspectRatioWidthValue = withFallback(desktopCardAspectRatioWidth, "3");
const desktopCardAspectRatioHeightValue = withFallback(desktopCardAspectRatioHeight, "4");
const tabletCardAspectRatioWidthValue = withFallback(tabletCardAspectRatioWidth, "3");
const tabletCardAspectRatioHeightValue = withFallback(tabletCardAspectRatioHeight, "4");
const mobileCardAspectRatioWidthValue = withFallback(mobileCardAspectRatioWidth, "3");
const mobileCardAspectRatioHeightValue = withFallback(mobileCardAspectRatioHeight, "4");

// group: { 4.1 Card Styling }
const cardSurfaceColor = $(cardSurfaceColor:Color="#F3ECE5");
const cardImageSurfaceColor = $(cardImageSurfaceColor:Color="#FFFFFF");
const cardLabelBackgroundColor = $(cardLabelBackgroundColor:Color="#FFFFFF");
// endgroup
...
// group: { 7.1 Card 1 }
const firstCardBackgroundImage = $(firstCardBackgroundImage:String="wkxx_2025_test_productstorycarousel_slidercopybottom_p1");
...
const firstCardTextBelow = $(firstCardTextBelow:RichText="";color="#000000");
// endgroup

// group: { 7.2 Card 2 }
const secondCardBackgroundImage = $(secondCardBackgroundImage:String="wkxx_2025_test_productstorycarousel_slidercopybottom_p2");
...
const secondCardTextBelow = $(secondCardTextBelow:RichText="";color="#000000");
// endgroup

// group: { 7.3 Card 3 }
const thirdCardBackgroundImage = $(thirdCardBackgroundImage:String="wkxx_2025_test_productstorycarousel_slidercopybottom_p3");
...
const thirdCardTextBelow = $(thirdCardTextBelow:RichText="";color="#000000");
// endgroup

// group: { 7.4 Card 4 }
const fourthCardBackgroundImage = $(fourthCardBackgroundImage:String="wkxx_2025_test_productstorycarousel_slidercopybottom_p4");
...
const fourthCardTextBelow = $(fourthCardTextBelow:RichText="";color="#000000");
// endgroup
...
const navigationColor = $(navigationColor:String="#000000");
const navigationBackgroundColor = $(navigationBackgroundColor:String="#ffffff");
// endgroup
...
const Button = ({
  ...
  if (variant === "solid") {
    return (
      <button
        type="button"
        className={`inline-flex items-center justify-center font-semibold tracking-wide belk-button px-6 py-3 rounded-full shadow-md hover:shadow-lg transition-shadow focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 ${className}`}
      >
  ...
  return (
    <button
      type="button"
      className={`inline-flex items-center gap-2 font-semibold tracking-wide belk-button ${className}`}
    >
...
const LeftArrow = ({ id, onClick, onPointerDown }) => (
  <button
    ...
    className={`flex absolute left-2 md:left-4 top-1/2 -translate-y-1/2 ${id}-carousel-navigation-color ${id}-carousel-navigation-background-color border-none rounded-full w-11 h-11 items-center justify-center cursor-pointer z-10 shadow-md hover:shadow-lg active:shadow transition-all`}
    aria-label="Previous items"
  >
...
const RightArrow = ({ id, onClick, onPointerDown }) => (
  <button
    ...
    onClick={onClick}
    onPointerDown={onPointerDown}
    className={`flex absolute right-2 md:right-4 top-1/2 -translate-y-1/2 ${id}-carousel-navigation-color ${id}-carousel-navigation-background-color border-none rounded-full w-11 h-11 items-center justify-center cursor-pointer z-10 shadow-md hover:shadow-lg active:shadow transition-all`}
    aria-label="Next items"
  >
    <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
 ...
 */
const cardBackgroundImageVarNames = ['firstCardBackgroundImage','secondCardBackgroundImage','thirdCardBackgroundImage','fourthCardBackgroundImage','fifthCardBackgroundImage','sixthCardBackgroundImage','seventhCardBackgroundImage','eighthCardBackgroundImage','ninthCardBackgroundImage','tenthCardBackgroundImage','eleventhCardBackgroundImage','twelfthCardBackgroundImage','thirteenthCardBackgroundImage','fourteenthCardBackgroundImage','fifteenthCardBackgroundImage','sixteenthCardBackgroundImage','seventeenthCardBackgroundImage','eighteenthCardBackgroundImage'];

const Card = ({ id, image, cta, textBelow, cardIndex }) => (
  <SafeLink className="flex-none snap-start cursor-pointer h-full" href={cta.url} data-aali={cta.adobeTag}>
    <article className={`flex h-full flex-col gap-4 p-4 shadow-lg ${cardRoundedCorners} ${id}-card-surface`}>
      <div data-bildit-var-name={cardBackgroundImageVarNames[cardIndex]} data-bildit-var-type="String">
        <div className={`relative w-full overflow-hidden ${cardRoundedCorners} ${id}-card-image-surface`}>
          <PictureResponsiveImage
            ...
            className={`flex w-full relative ${id}-aspect-[mobile-card] md:${id}-aspect-[tablet-card] xl:${id}-aspect-[desktop-card]`}
          />
        </div>
      </div>
      {textBelow.content && (
        <div className={`flex items-center justify-center rounded-xl px-4 py-3 ${id}-card-label-surface`}>
          <p className={`${id}-belk-text-clamp-text-below text-center line-clamp-2`}>{textBelow.content}</p>
        </div>
      )}
    </article>
  </SafeLink>
    ...
    @layer template {
      :host, :root {
        @layer container {
          ...
          --${id}-inner-container-background-secondary: ${innerContainerSecondaryBackgroundColor};
          --${id}-container-font-family: ${fontFamily};
          --${id}-desktop-container-aspect-ratio: ${desktopContainerAspectRatioWidthValue} / ${desktopContainerAspectRatioHeightValue};
          --${id}-tablet-container-aspect-ratio: ${tabletContainerAspectRatioWidthValue} / ${tabletContainerAspectRatioHeightValue};
          --${id}-mobile-container-aspect-ratio: ${mobileContainerAspectRatioWidthValue} / ${mobileContainerAspectRatioHeightValue};
        }
        @layer eyebrow {
        ...
        @layer cards {
          --${id}-mobile-card-aspect-ratio: ${mobileCardAspectRatioWidthValue} / ${mobileCardAspectRatioHeightValue};
          --${id}-tablet-card-aspect-ratio: ${tabletCardAspectRatioWidthValue} / ${tabletCardAspectRatioHeightValue};
          --${id}-desktop-card-aspect-ratio: ${desktopCardAspectRatioWidthValue} / ${desktopCardAspectRatioHeightValue};
          --${id}-card-surface-color: ${cardSurfaceColor};
          --${id}-card-image-surface-color: ${cardImageSurfaceColor};
          --${id}-card-label-background-color: ${cardLabelBackgroundColor};
          --${id}-text-below-font-size: clamp(0.85rem, 1.2dvw, 1rem);
        }
        @layer carousel {
...
const CardStyles = ({ id }) => (
  <style>{`
    ...
    .${id}-card-surface {
      background: var(--${id}-card-surface-color);
    }
    .${id}-card-image-surface {
      background: var(--${id}-card-image-surface-color);
    }
    .${id}-card-label-surface {
      background: var(--${id}-card-label-background-color);
    }
  `}</style>
)
...
const ProductStoryCarousel = ({ id }) => {
  ...
  const cards = cardData
    .filter(([slug]) => slug)
    .map(([slug, altText, preset, url, adobeTag, textBelow]) => ({
      image: { slug, altText, preset },
      cta: { url, adobeTag },
      textBelow: { content: textBelow }
    }));

  const hasInnerBackgroundImage = Boolean(innerContainerBackgroundImage);
  const innerContainerStyle = hasInnerBackgroundImage ? undefined : {
    background: `linear-gradient(105deg, var(--${id}-inner-container-background-color) 0%, rgba(255,255,255,0.92) 45%, var(--${id}-inner-container-background-secondary) 100%)`
  };
  ...
  return (
    <>
      ...
      >
        <div data-bildit-var-name="outerContainerBackgroundImage" data-bildit-var-type="String">
        <PictureResponsiveImage
          images={{
          ...
          className={`relative justify-center ${roundedCornersTop} ${roundedCornersBottom} p-4 md:p-6 xl:p-10`}
          style={{ backgroundColor: outerContainerBackgroundColor }}
        >

          <div data-bildit-var-name="innerContainerBackgroundImage" data-bildit-var-type="String">
          <PictureResponsiveImage
            images={{
            ...
            imageClassName="object-cover object-center"
            className={`${innerContainerRoundedCorners} ${id}-inner-container-color ${id}-inner-container-font-family relative justify-center px-4 py-5 sm:px-5 sm:py-6 md:px-6 md:py-6 xl:px-10 xl:py-10 overflow-visible shadow-lg`}
            style={innerContainerStyle}
          >
            <div className="flex flex-col gap-6 md:flex-row md:items-stretch md:gap-8 overflow-visible">
              <div className="flex w-full max-w-md flex-col justify-center gap-3 text-left">
                <div className="space-y-2">
                  <p className={`${id}-belk-text-clamp-eyebrow font-semibold`}>{eyebrowContent}</p>
                  <h2 className={`${id}-belk-text-clamp-headline font-bold leading-tight`}>{headlineContent}</h2>
                  <h3 className={`${id}-belk-text-clamp-subhead text-black/70`}>{subheadContent}</h3>
                </div>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                  {ctaContent && (
                    ...
                    >
                      <Button
                        className={`${id}-belk-text-clamp-cta ${id}-cta-background-color cursor-pointer w-full sm:w-auto`}
                        variant={ctaVariant}
                      >
                        {ctaContent}
                      </Button>
                  ...
                  {secondaryCtaContent && (
                    <SafeLink
                      ...
                      data-aali={secondaryCtaAdobeTag}
                    >
                      <Button
                        className={`${id}-belk-text-clamp-cta text-left text-black w-full sm:w-auto`}
                        variant="underline"
                      >
                        {secondaryCtaContent}
                      </Button>
              ...
              <div className="relative flex w-full flex-1 min-w-0 md:pl-6 lg:pl-10">
                <Carousel
                  interval={interval * 1000}
                  autoplay={Boolean(autoplay)}
                  loop={Boolean(loop)}
                  perPage={{ xs: 1.05, md: 1.25, lg: 3 }}
                  pagination={Boolean(pagination)}
                  navigation={{xs: true, lg: true}}
                  perMove={1}
                  gap={18}
                  padding={{xs: { right: 32 }, md: { right: 48 }, lg: { right: 120 } }}
                  ...
                  className="w-full"
                >
                  {cards.map(({image, cta, textBelow}, index) => (
                    <Card
                      id={id}
                      key={index}
                      image={image}
...
export default ProductStoryCarousel;