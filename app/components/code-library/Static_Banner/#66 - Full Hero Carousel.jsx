...
const ctaBgColor = $(ctaBgColor:Color="#203048");
const ctaText = $(ctaText:String="Shop Back to School");
const ctaUrl = $(ctaUrl:String);
const ctaAdobeTag = $(ctaAdobeTag:String);
// endgroup

// group {Images}
const imageSlug = $(imageSlug:String="wk25_072125_newcms_homepage_bts_slider_1");
const imageSlug2 = $(imageSlug2:String="wk25_072125_newcms_homepage_bts_slider_2");
const imageSlug3 = $(imageSlug3:String="wk25_072125_newcms_homepage_bts_slider_3");
const imagePreset = "$DWP_PHOTO$";
const headlineImage = $(headlineImage:String="wk25_072125_newcms_homepage_bts_headline");
...
const styles = `
  .${cn("wrapper")} {
    ...
    .${cn("carousel-link")} {
      ...
      .${cn("bg-image-2")} {
        @media (max-width: 767px) {
          background-image: ${isValidImageUrl(imageSlug2) ? `url(${buildImageUrl(imageSlug2, imagePreset, true)})` : "none"};
        }
        @media (min-width: 768px) {
          background-image: ${isValidImageUrl(imageSlug2) ? `url(${buildImageUrl(imageSlug2, imagePreset, false)})` : "none"};
        }
      }

      .${cn("bg-image-3")} {
        @media (max-width: 767px) {
          background-image: ${isValidImageUrl(imageSlug3) ? `url(${buildImageUrl(imageSlug3, imagePreset, true)})` : "none"};
        }
        @media (min-width: 768px) {
          background-image: ${isValidImageUrl(imageSlug3) ? `url(${buildImageUrl(imageSlug3, imagePreset, false)})` : "none"};
        }
      }
      ...
      .${cn("copy")} {
        ...
        .${cn("headlineImage")} {
          ...
          background-repeat: no-repeat;
          @media (max-width: 767px) {
            background-image: ${isValidImageUrl(headlineImage) ? `url(${buildImageUrl(headlineImage, headlineImagePreset, true)})` : "none"};
          }
          @media (min-width: 768px) {
            background-image: ${isValidImageUrl(headlineImage) ? `url(${buildImageUrl(headlineImage, headlineImagePreset, false)})` : "none"};
          }
        }
...
const FullHeroCarousel66StaticTemplate = () => {
        ...
        >
          ...
          <div className={cn("copy")}>
            <div
              className={`${cn(
                "headlineImage"
              )} aspect-[226/72] md:aspect-[936/295]`}
            ></div>
            <div
              className={`${cn("cta")} ${getFontSizeClass(
                "l",
                "m"
...
export default FullHeroCarousel66StaticTemplate;