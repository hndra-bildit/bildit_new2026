...
export function HomeHero({
  ...
  // group { 1. div }
  const breakFreeFromDeploymentBottlenecksText = $(breakFreeFromDeploymentBottlenecksText:RichText={ text: "Break free from deployment bottlenecks." });
  const fasterText = $(fasterText:RichText={ text: "Faster" });
  const link1 = $(link1:RichText={ text: "Start Building" });
  const roundedCorners = $(roundedCorners:Boolean=true);
  const shipHighPerformanceText = $(shipHighPerformanceText:RichText={ text: "Ship high performance" });
  const span1Span = $(span1Span:RichText={ text: "experiences." }); // endgroup
  return <section id="home-hero" data-header-surface="dark" className={cn('relative m-0 flex h-svh max-h-svh w-full flex-col overflow-hidden rounded-none sm:mt-[calc((1rem+20px)*0.42-10px)] sm:mb-[calc((1rem+20px)*0.315)] sm:ml-[calc((1rem+20px)*0.42-10px)] sm:mr-[calc((1rem+20px)*0.42-10px)] sm:h-[calc(100svh-(1rem+20px)*0.735+10px)] sm:max-h-[calc(100svh-(1rem+20px)*0.735+10px)] sm:w-auto sm:rounded-[29px] sm:shadow-[0_0_5px_rgba(0,0,0,0.3)]', className)}>
      <HeroFloatingLines />
      <SiteHeroTopSpacer />
      <div className="relative z-10 flex min-h-0 flex-1 flex-col overflow-hidden">
        <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-14 px-4 py-16 text-center sm:px-10 sm:py-20 md:gap-[60px]">
          <div className="flex max-w-[896px] flex-col gap-5 md:gap-5">
            ...
            <p className="font-[family-name:var(--font-uncut-sans)] text-lg font-light leading-relaxed text-white/85 sm:text-xl md:text-2xl md:leading-[39px]" data-bildit-var-name="breakFreeFromDeploymentBottlenecksText" data-bildit-var-type="RichText">
              {breakFreeFromDeploymentBottlenecksText}
            </p>
          </div>
          <Link href={BILDIT_SIGNUP_URL} className={`font-[family-name:var(--font-uncut-sans)] inline-flex h-11 items-center justify-center gap-2.5 bg-gradient-to-r from-[#c850f0] to-[#e84590] px-[19px] py-2.5 text-base font-semibold text-white shadow-lg transition-transform hover:scale-[1.02] active:scale-[0.98] ${roundedCorners ? "rounded-full" : ""}`} data-bildit-var-name="roundedCorners" data-bildit-var-type="Boolean">
            {link1}
