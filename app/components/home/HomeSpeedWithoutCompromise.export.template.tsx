...
import { homeSectionSubtitleClassName, homeSectionTitleClassName } from "@/cmsDependencies";
...
export function HomeSpeedWithoutCompromise({
  ...
  return <section className={cn('home-scheme-light relative w-full overflow-hidden rounded-b-[64px] bg-white text-neutral-900', className)}>
      <div className="pointer-events-none absolute inset-x-0 top-[72px] flex justify-center px-6 sm:px-8" aria-hidden>
        <div className="relative h-[280px] w-full max-w-[min(560px,calc(100%-2rem))]">
          <div className={`absolute left-[18%] top-[52%] size-[220px] -translate-x-1/2 -translate-y-1/2 bg-[rgba(124,58,237,0.14)] blur-[72px] ${sectionDivRoundedCornersToggle ? "rounded-full" : ""}`} />
          <div className={`absolute left-[58%] top-[38%] size-[200px] -translate-x-1/2 -translate-y-1/2 bg-[rgba(232,69,144,0.085)] blur-[72px] ${sectionDivRoundedCornersToggle ? "rounded-full" : ""}`} />
        </div>
      </div>
      ...
      <div className="relative mx-auto flex max-w-[1512px] flex-col items-center gap-12 px-3 py-16 sm:px-4 md:gap-16 md:py-20 lg:py-24">
        <header className="flex max-w-[768px] flex-col items-center gap-4 text-center">
          <h2 className={cn('text-center', homeSectionTitleClassName)}>{headerH2Text}</h2>
          <p className={cn(homeSectionSubtitleClassName, 'text-center')}>{headerPText}</p>
        </header>

        <div className="grid w-full grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-8 xl:grid-cols-4 xl:gap-[34px]">
          {CARDS.map(card => <article key={card.id} className={`flex min-w-0 flex-col gap-3.5 overflow-hidden border border-black/10 bg-white ${divArticleRoundedCornersToggle ? "rounded-2xl" : ""}`}>
              <div className={`relative w-full overflow-hidden aspect-[${articleDivAspectRatioWidthNumber}/${articleDivAspectRatioHeightNumber}]`}>
                <Image src={card.imageSrc} alt={card.imageAlt} fill sizes="(max-width: 640px) 100vw, (max-width: 1280px) 50vw, 25vw" className="object-cover object-center" />
              </div>
              <div className="flex flex-col gap-3.5 px-2.5 pb-2.5">
                {card.iconSrc ? <div className={`flex size-12 shrink-0 items-center justify-center px-3 ${articleDivRoundedCornersToggle ? "rounded-xl" : ""}`} style={{
