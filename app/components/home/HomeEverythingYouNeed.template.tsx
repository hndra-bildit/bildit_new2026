...
const PRIMARY_ITEMS = ['Visual Experience Engine', 'Instant publishing infrastructure', 'Performance guardrails built-in', 'Sophisticated template system', 'Live preview that actually works'] as const;
const BONUS_ITEMS = ['Elevated template starter pack', 'White-glove onboarding', 'Direct access to our engineering team'] as const;
  ...
  return <section ref={sectionRef} className={cn('home-scheme-light relative w-full overflow-hidden bg-white', className)}>
      <div className="absolute inset-0 overflow-hidden" aria-hidden>
        ...
        transform: `translateY(${parallaxY}px)`
          ...
          <img src={SECTION_BACKGROUND_SRC} alt="" className="absolute inset-0 h-full w-full object-cover object-center" aria-hidden />
        </div>
      </div>

      <div aria-hidden className={`relative z-[1] w-full -none bg-white sm: -[51px] ${sectionDivRoundedCornersToggle ? "rounded-b" : ""}`} style={{
      height: CAP_HEIGHT_PX
    }} />

      <div className={cn('relative z-[1] -mt-px w-full overflow-hidden shadow-[0px_24px_80px_rgba(13,1,24,0.12)]')}>
        <div className={`pointer-events-none absolute bottom-0 left-1/2 z-0 size-[min(600px,90vw)] -translate-x-1/2 translate-y-1/4 bg-[rgba(200,80,240,0.07)] blur-[140px] ${sectionDivRoundedCornersToggle ? "rounded-full" : ""}`} aria-hidden />

        <div className="relative z-[1] px-0 py-14 sm:px-8 sm:py-16 md:px-10 md:py-20 lg:px-[116px] lg:py-24">
          <div className={`relative mx-auto flex min-h-0 w-full max-w-[1280px] flex-col items-center overflow-hidden px-5 py-10 sm: -[24px] sm:p-8 md:p-12 lg:min-h-[720px] lg:px-[116px] lg:py-16 ${sectionDivRoundedCornersToggle ? "rounded-none" : ""}`}>
            <div className={`absolute inset-0 overflow-hidden sm: -[24px] ${sectionDivRoundedCornersToggle ? "rounded-none" : ""}`}>
              <Image src={divImageSrcValue} alt="" fill className="object-cover object-center" sizes="(max-width: 1280px) 100vw, min(1280px, 100vw)" />
            ...
            <div className={cn('relative z-[1] mt-10 w-full max-w-[829px] rounded-2xl border border-black/[0.08]', 'bg-white p-8 md:p-10')}>
      ...
      <div aria-hidden className={`relative z-[1] -mt-px w-full -none bg-white sm: -[51px] ${sectionDivRoundedCornersToggle ? "rounded-t" : ""}`} style={{
      height: CAP_HEIGHT_PX
    }} />
    </section>;
}