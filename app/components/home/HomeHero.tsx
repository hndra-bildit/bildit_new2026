 ...
 * Site header + mega menu live at the top of this section. Inset 15px from all viewport edges.
...
export function HomeHero({ className }: { className?: string }) {
  return (
    <section
      id="home-hero"
      className={cn('relative m-[15px] flex min-h-[calc(100svh-30px)] w-auto flex-col rounded-[51px]', className)}
    >
      <HeroFloatingLines />
      <SiteHeader variant="inHero" />
      <div className="relative z-10 flex flex-1 flex-col items-center justify-center gap-14 px-4 py-16 text-center sm:px-10 sm:py-20 md:gap-[60px]">
        <div className="flex max-w-[896px] flex-col gap-5 md:gap-5">
          <h1 className="font-[family-name:var(--font-inter)] text-4xl font-bold leading-[1.05] text-white sm:text-5xl md:text-6xl lg:text-[72px] lg:leading-[72px]">
          ...
          <p className="font-[family-name:var(--font-inter)] text-lg leading-relaxed text-white sm:text-xl md:text-2xl md:leading-[39px]">
            Break free from deployment bottlenecks.
          </p>
        </div>
        <Link
          href="/pricing/"
          className="font-[family-name:var(--font-inter)] inline-flex h-11 items-center justify-center gap-2.5 rounded-full bg-gradient-to-r from-[#c850f0] to-[#e84590] px-[19px] py-2.5 text-base font-semibold text-white shadow-lg transition-transform hover:scale-[1.02] active:scale-[0.98]"
        >
          Start Building
