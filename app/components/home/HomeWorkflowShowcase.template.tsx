// HomeWorkflowShowcase:v1.0 legacy=true
'use client';
...
export function HomeWorkflowShowcase({
  ...
  // group { 1. div }
  const divP = $(divP:RichText='Build. Preview. Publish.... Fast');
  const h2 = $(h2:RichText='Your content is visual. Build visually.'); // endgroup
  ...
  return <section className={cn('home-scheme-light relative overflow-hidden bg-white', className)}>
      <div className="relative mx-auto flex max-w-[1286px] flex-col gap-10 px-6 py-16 md:gap-[50px] md:px-10 md:py-20 lg:px-[116px] lg:py-24">
        <div className="flex flex-col items-center gap-2.5 text-center">
          <h2 className={cn('text-balance', homeSectionTitleClassName)} data-bildit-var-name="h2" data-bildit-var-type="RichText">{h2}</h2>
          <p className={cn(homeSectionSubtitleClassName, 'text-center')} data-bildit-var-name="divP" data-bildit-var-type="RichText">{divP}</p>
        </div>
        ...
        <div className={cn('flex w-full flex-col gap-6 md:max-w-none md:gap-8 lg:gap-[60px]', showSocialStrip ? 'md:flex-row md:items-center' : 'md:items-center')} data-name="Main Frame">
          <div className={cn('flex w-full shrink-0 flex-col gap-2.5', showSocialStrip ? 'items-start md:w-[300px]' : 'items-center text-center')}>
            <p className={cn(homeSectionSubtitleClassName, 'max-w-[250px]')} id="home-workflow-cta-heading" data-bildit-var-name="divP" data-bildit-var-type="RichText">
              {divP}
            </p>
