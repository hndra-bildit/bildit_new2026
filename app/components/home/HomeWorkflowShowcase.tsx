...
import { HowItWorksInteractiveShowcase } from '@/app/components/workflow/HowItWorksInteractiveShowcase'
...
type HomeWorkflowShowcaseProps = {
  ...
  /** Uses the imported scroll-driven "How it works" experience from the landing redesign. @default false */
  showImportedHowItWorks?: boolean
}

export function HomeWorkflowShowcase({
  className,
  showSocialStrip = true,
  showImportedHowItWorks = false
}: HomeWorkflowShowcaseProps) {
  const [active, setActive] = useState(0)

  return (
    <section
      className={cn(
        'home-scheme-light relative bg-white',
        showImportedHowItWorks ? 'overflow-x-clip' : 'overflow-hidden',
        className
      )}
    >
      <div
        className={cn(
          'relative mx-auto flex flex-col gap-10 px-6 py-16 md:gap-[50px] md:px-10 md:py-20 lg:py-24',
          showImportedHowItWorks ? 'max-w-[1512px] lg:px-4 xl:px-6' : 'max-w-[1286px] lg:px-[116px]'
        )}
      >
        ...
        {showImportedHowItWorks ? (
          <HowItWorksInteractiveShowcase />
        ) : (
          <>
            ...
            >
              ...
              {showSocialStrip ? (
                <HomeWorkflowSocialStrip className="w-full min-w-0" id="home-workflow-social" />
              ) : null}
            </div>
          </>
        )}
