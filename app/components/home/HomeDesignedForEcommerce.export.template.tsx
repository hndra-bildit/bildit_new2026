...
function StatSevenGradient({
  ...
  return <p className={cn(VALUE_CLASS_METRIC, 'home-stat-metric-wrap font-bold tabular-nums', active && 'home-stat-metric-fill', className)} aria-label="7× faster publishing">
      {pText}
    </p>;
}
function StatPlusThirtyThree({
  ...
  return <p className={cn(VALUE_CLASS_METRIC, 'home-stat-metric-wrap font-semibold tabular-nums', active && 'home-stat-metric-fill', className)} aria-label="+33% faster page loads">
      {pText}
    </p>;
}
function StatZero({
  ...
  return <p className={cn(VALUE_CLASS_METRIC, 'home-stat-metric-wrap font-bold tabular-nums', active && 'home-stat-metric-fill', className)}>
      {pText}
    </p>;
}
...
export function HomeDesignedForEcommerce({
  ...
  return <section className={cn('home-scheme-light relative w-full overflow-hidden bg-white text-neutral-900', className)}>
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        <div className={`absolute left-[8%] top-[18%] size-[380px] bg-[rgba(232,69,144,0.06)] blur-[100px] md:left-[12%] ${sectionDivRoundedCornersToggle ? "rounded-full" : ""}`} />
        <div className={`absolute right-[5%] top-[35%] size-[420px] bg-[rgba(200,80,240,0.055)] blur-[110px] md:right-[8%] ${sectionDivRoundedCornersToggle ? "rounded-full" : ""}`} />
        <div className={`absolute bottom-[12%] left-1/2 size-[400px] -translate-x-1/2 bg-[rgba(255,68,102,0.045)] blur-[100px] ${sectionDivRoundedCornersToggle ? "rounded-full" : ""}`} />
      </div>
      <div className="relative mx-auto w-full max-w-[1286px] px-6 py-20 md:px-10 md:py-[100px] lg:px-[116px]">
        <div className="flex flex-col items-center gap-5 text-center">
          <h2 className={cn('max-w-[855px] text-center', homeSectionTitleClassName)}>{divH2Text}</h2>
