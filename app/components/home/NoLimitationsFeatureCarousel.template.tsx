// NoLimitationsFeatureCarousel:v2 legacy=false
// @template v2
...
export function NoLimitationsFeatureCarousel({
   ...
   * @type Number
   */
  const aspectRatioHeight = 260
  /**
   * @group General
   * @type Number
   */
  const aspectRatioWidth = 402
  /**
   * @group General
   * @type Boolean
   */
      ...
      <div className={cn('mx-auto flex w-max gap-5', trackPaddingClassName)}>
        {NO_LIMITATIONS_FEATURE_CARDS.map((card, index) => {
          ...
          return (
            ...
            >
              <div
                className={`relative w-full overflow-hidden -2xl bg-neutral-100 ${roundedCorners ? 'rounded-t' : ''}`}
